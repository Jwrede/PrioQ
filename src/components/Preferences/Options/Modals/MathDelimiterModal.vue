  ...
  <div ref="modalPopup" class="modal_popup" id="modal_popup">
    <form action="" class="zooming" method="post">
      ...
      <div id="add-container">
        ...
        <div>Right:</div>
        <div>Display:</div>
        <input id="left-input" class="text" type="text" spellcheck="false" />
        <input
          id="right-input"
          @keydown="addOnEnter"
          class="text"
        ...
        />
        <div id="checkbox">
          <input id="display-input" type="checkbox" />
          <label for="display-input" class="check-trail">
            <span class="check-handler"></span>
          </label>
        </div>
      </div>
      <div id="button-container">
        <a class="button" id="accept" @click="add">Accept</a>
...
<script>
export default {
  name: "MathDelimiterModal",
  methods: {
    add() {
      let items = Array.from(document.getElementsByClassName("text"));
      let allValid = true;
      items.forEach((input) => {
        if (input.value === "") {
      ...
      if (allValid) {
        ...
        let display = document.getElementById("display-input").checked;
        let delimiter = {
          left: left,
          right: right,
          display: display,
        };
        this.$emit("addDelimiter", delimiter);
